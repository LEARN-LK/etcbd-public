...
* The system should be running a recent version of Linux supported by Docker.  For [Ubuntu](https://docs.docker.com/engine/installation/linux/ubuntulinux/), this is 22.04.
...
On each of the VMs, start by cloning the git repository:
    ...
    git clone https://github.com/LEARN-LK/etcbd-public
...
* ````REALM_EXISTING_DATA_URL````: In a real deployment, leave this set to blank
  ...
  (e.g., you can load the REANNZ data by setting the URL to `https://member.DOMAIN.ac.lk/general/institution.xml`).
  ...
  * Choose from:
    ...
    * locallogin: Local accounts on the admin tool instance.
       * Note that local accounts can later be created by logging into the
         Admintool at https://admin.YOUR-DOMAIN.ac.lk/admin/ as the administrator
...
* `SITE_PUBLIC_HOSTNAME`: if this value has changed after running `admintool-setup.sh`, besides restarting the containers, change the value also in the Sites entry in the management interface at https://admin.YOUR-DOMAIN.ac.lk/admin/
* `REALM_COUNTRY_CODE`, `NRO_INST_NAME`: if any of these values has changed after running `admintool-setup.sh`, besides restarting the containers, change the value also in the Realms entry in the management interface at https://admin.YOUR-DOMAIN.ac.lk/admin/
* `ADMIN_USERNAME`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`: if any of these values has changed after running `admintool-setup.sh`, besides restarting the containers, change the value also in the account definition in the Users table in the management interface at https://admin.YOUR-DOMAIN.ac.lk/admin/
...
* Log into the Admintool admin interface at https://admin.YOUR-DOMAIN.ac.lk/admin/ as the administrator (with the username and password created earlier).
...
Now you should be able to access the monitoring configuration at https://admin.YOUR-DOMAIN.ac.lk/icingaconf with this account.  (And it is also accessible under the administrator account).
...
All of the data can be entered at https://admin.YOUR-DOMAIN.ac.lk/admin/
 ...
 * `CONF_URL_LIST`: the URL (possibly a list of URLs) to fetch configuration from.  Should be https://eduroam-admin.YOUR-DOMAIN.ac.lk/icingaconf (If you do not have a proper SSL certificate, icingaconf will not be loaded to icinga instance)
...
* `ICINGAWEB2_ADMIN_USERNAME`, `ICINGAWEB2_ADMIN_PASSWORD`: if any of these values has changed after running `icinga-setup.sh`, change the value also in the account definition in the management interface at https://monitoring.YOUR-DOMAIN.ac.lk/icingaweb2/ (navigate to Configuration => Authentication => Users => select the account).
...
* https://metrics.YOUR-DOMAIN.ac.lk:9443/app/kibana#/dashboard/Aggregate-Trends
* https://metrics.YOUR-DOMAIN.ac.lk:9443/app/kibana#/dashboard/Aggregate-Trends-and-Intl-Visitors
* https://metrics.YOUR-DOMAIN.ac.lk:9443/app/kibana#/dashboard/InstitutionalTrendAndActivity_<inst-dns-name>
...
The Admintool can be accessed at https://admin.YOUR-DOMAIN.ac.lk/

The management interface of the Admnintool can be accessed at https://admin.YOUR-DOMAIN.ac.lk/admin/

The management interface of the monitoring tools (Icingaweb2) can be accessed at https://monitoring.YOUR-DOMAIN.ac.lk:8443/icingaweb2/

The web interface of the metrics tools (Kibana) can be accessed at https://metrics.YOUR-DOMAIN.ac.lk:9443/
...
Now you should be able to access the monitoring configuration at https://admin.YOUR-DOMAIN.ac.lk/radsecproxyconf with this account.  (And it is also accessible under the administrator account).
